  ...
  if (points.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-6 border border-cf-border/40 rounded-xl bg-white/5 text-center">
        ...
        <p className="text-xs text-white/30 mt-1">Trigger Pythagoras diagnostic to extract microtonal performance telemetry.</p>
      </div>
    );
  }
    ...
    <div className="pitch-telemetry-container bg-cf-deep border border-cf-border/60 rounded-xl p-4">
      ...
      <div className="relative overflow-x-auto">
        <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} className="overflow-visible">
          ...
          />
          {/* Zero line tag */}
          <text x={padding + 5} y={height / 2 - 5} fill="rgba(201,169,110,0.5)" className="text-[8px] font-mono">
          ...
          {points.map((pt, idx) => {
            ...
            const dotColor = isAccurate ? '#c9a96e' : '#7b6aaa'; // Gold if accurate, violet if flat/sharp
            const glowColor = isAccurate ? 'rgba(201,169,110,0.4)' : 'rgba(123,106,170,0.4)';
