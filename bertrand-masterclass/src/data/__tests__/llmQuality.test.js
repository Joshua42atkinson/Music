...
describe('LLM Quality — Automated Scoring', () => {
  test('all test cases have required fields', () => {
    ...
    for (const tc of cases) {
      ...
      expect(tc.mode).toMatch(/chat|troubadour/);
  ...
  test('Over protocol scoring — troubadour mode', () => {
    expect(scoreOverProtocol('Practice slowly today. Over.', 'troubadour')).toBe(5);
    expect(scoreOverProtocol('Practice slowly. Over. Remember to breathe.', 'troubadour')).toBe(3);
    expect(scoreOverProtocol('Practice slowly today.', 'troubadour')).toBe(1);
  });
