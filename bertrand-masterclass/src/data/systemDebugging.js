...
export const SYMPTOM_FIX_MAP = {
  // ── AI / Troubadour Issues ──
  'Troubadour is silent': {
    ...
    fix: 'TroubadourWidget.jsx → speakTextInternal() → check kokoroRef.current.isReady → fallback to Web Speech API',
    fallback: 'If all TTS fails, text still appears in chat. Student can read.',
  },
  'Troubadour says "Over" in text chat': {
    likelyCause: 'Mode mismatch — troubadour voice mode active while typing',
    check: [
      'Is microphone active? (red dot = listening)',
      'Did student toggle voice mode by mistake?',
    ],
    fix: 'troubadourPrompt.js → enforceOver() → check mode param. If mode===chat, strip "Over."',
    code: 'enforceOver(text, "chat") should NOT append "Over."',
  },
  'Troubadour gives wrong fret advice': {
    ...
    fix: 'TroubadourWidget.jsx → buildChatPrompt() → verify currentFret/currentPhase passed correctly',
  },
  'AI responses are too long': {
    ...
    fix: 'useTroubadourAI.js → chatStream() → enforce max_tokens limit per backend',
  },
  ...
  'Wllama shows "Init failed"': {
    ...
    fix: 'useWllamaTroubadour.js → initEngine() → check retry count → if max retries, suggest 350m fallback',
  ...
  'Wllama stuck at 0% loading': {
    ...
    fix: 'useWllamaTroubadour.js → WASM_PATHS → try local fallback if CDN fails',
  },
  ...
  'TTS repeats or overlaps': {
    ...
    fix: 'useTroubadourAI.js → speakTextInternal() → cancel() before new speak',
  },
...
export const FIX_AT_CHAINS = {
  ...
  'dagNodes.js': [
    ...
    'Check troubadourPrompt field on the node',
  ...
  'TroubadourWidget.jsx': [
    ...
    'Verify mode state: chat vs troubadour (Over. protocol)',
  ...
  'PlayerPortal.jsx': [
    ...
    'Verify Troubadour Type override in settings',
  ],
  'SlideViewer.jsx': [
...
export const KNOWN_BUGS = [
  ...
  {
    id: 'BUG-007',
    symptom: 'TroubadourWidget is a monolith (~600 lines)',
    status: 'TECH_DEBT',
