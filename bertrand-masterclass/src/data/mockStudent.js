...
// so the Troubadour AI can be tested without real users.
// ═══════════════════════════════════════════════════════════════════

import { FRET_METADATA } from './dag/dagNodes';

export const TROUBADOUR_TYPES = ['storyteller', 'craftsman', 'ear', 'seeker'];
...
function _generate(seed) {
  ...
  const type = TROUBADOUR_TYPES[rInt(0, 3)];
...
export function injectMockStudent(student = null) {
  ...
  localStorage.setItem('voixvive_mock_student', JSON.stringify(s));
  localStorage.setItem('voixvive_practice_log', JSON.stringify(s.practiceLog));
  // Merge into traction store
  const existing = JSON.parse(localStorage.getItem('voixvive_traction') || '{}');
  localStorage.setItem('voixvive_traction', JSON.stringify({
    ...existing,
    ...
    studentProfile: { troubadourType: s.troubadourType },
  }));
  return s;
}
...
export function clearMockStudent() {
  localStorage.removeItem('voixvive_mock_student');
}
