...
import { getChapterProgress, getSlidePosition } from './localDatabase';
...
export const CHAPTER_TOOL_MAP = {
  ...
  4:  { toolId: 4,  name: "Troubadour's Quill",   why: 'Can I express what I feel?' },
...
export function getCurriculumPosition() {
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    if (!fret) continue;
    const totalSlides = generateSlides(fret).length;
...
export function getPracticeContext() {
  ...
  const lastFretPracticed = (() => {
    try {
      return parseInt(localStorage.getItem('voixvive_last_tool_fret')) || null;
    } catch { return null; }
  })();
