...
const PROMPT_VERSIONS_KEY = 'voixvive_prompt_versions';
const PROMPT_RESULTS_KEY = 'voixvive_prompt_results';
...
export function getPromptVersions() {
  try {
    return JSON.parse(localStorage.getItem(PROMPT_VERSIONS_KEY)) || [];
  } catch {
    return [];
  }
}
...
export function savePromptResults(results) {
  ...
  localStorage.setItem(PROMPT_RESULTS_KEY, JSON.stringify(allResults));
}
...
export function getPromptResults() {
  try {
    return JSON.parse(localStorage.getItem(PROMPT_RESULTS_KEY)) || [];
  } catch {
    return [];
  }
}
