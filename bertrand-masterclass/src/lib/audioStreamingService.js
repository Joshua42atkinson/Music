...
const STEP_MIDDLEWARE_URL = import.meta.env.VITE_STEP_MIDDLEWARE_URL || 'ws://localhost:8081/ws/troubadour';
...
class AudioStreamingService {
  ...
  async connect(url = STEP_MIDDLEWARE_URL, authToken = null) {
    return new Promise((resolve, reject) => {
      try {
        ...
        this.ws.onopen = () => {
          console.log('[AudioStreaming] Connected to', url);
        ...
        this.ws.onclose = () => {
          console.log('[AudioStreaming] Disconnected');
  ...
  updateSession({ persona = 'troubadour', fretId = 1, modalities = ['text', 'audio'] }) {
    if (this.ws?.readyState !== WebSocket.OPEN) return;

    const personas = {
      troubadour: "You are the Troubadour, a Socratic guitar mentor. Teach using ©SHEARL, ©PLING!, ©FHEAL. Speak slowly, contemplatively. Help students discover, not just instruct.",
      bernard: "You are Bernard de Ventadorn, 12th-century troubadour. You speak in poetic metaphors about music, love, and the soul. You are gentle, wise, and slightly melancholic.",
    ...
    this.ws.send(JSON.stringify({
      ...
      session: {
        modalities,
        instructions: personas[persona] || personas.troubadour,
