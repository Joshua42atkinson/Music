...
## 7. The AI Delivery System — Three-Layer Architecture

> **Full design:** `docs/07_MINIMUM_AI_MODE.md`

Kriya's workbook is designed so a sincere student can progress alone. The guru is internalized through practice itself. Voix Vive's AI system must follow the same principle: **the lowest tier must still teach.**

### 7.1 Kriya's Three Levels → Voix Vive's Three AI Tiers

| Kriya Level | Mantra Practice | Voix Vive AI Tier | What the Student Gets |
|-------------|----------------|-------------------|----------------------|
| **Level 1** (aloud) | External sound guides practice | **Souffle** (0 MB, offline) | Static prompts + Web Speech TTS. The book speaks. No server, no download. |
| **Level 2** (silent) | Internalized, adapts to the moment | **Voix** (~2.65 GB, in-browser) | LFM2.5-1.2B GGUF via wllama + Kokoro-82M TTS. The guide adapts. Toggle 🔮/🤫. |
| **Level 3** (autonomous) | Practice IS the practitioner | **Chant** (server-required) | StepAudio 33B + full prompt engineering + voice stream. Bertrand embodied. |

### 7.2 Audio Is Always On (The Game's Mantra)

Kriya's mantra is always running — it's not something you turn off. Voix Vive's game audio follows the same principle:

- Breathing gate audio cues → always active
- Metronome → always active
- Pitch reference tones → always active
- Troubadour voice (TTS) → always active (quality varies by tier)

The `aiEnabled` toggle controls the **LLM chat** only. Audio continues regardless. This mirrors Kriya: even at Level 1, the mantra is always present. You can't "turn off" the breath.

### 7.3 The Workbook IS the Guru — The Whisper Test

**Kriya's design constraint:** *"Do not wait for advice to practice Kriya."* The book alone suffices.

**Voix Vive's equivalent constraint:** A student using only Layer 1 (Whisper — no server, no download, no AI toggle) must still receive meaningful pedagogical guidance. The static prompts in `troubadourOffline.js` must be rich enough to guide practice. If they aren't, the architecture fails the Kriya test.

### 7.4 Scaffolding Fade → AI Fade

Kriya's deepest form of scaffolding fade: the book gradually becomes unnecessary as the student's inner guidance strengthens. The same should happen with the AI:

| Mastery Level | Kriya Stage | AI Behavior |
|--------------|-------------|-------------|
| Encountered (0) | Mantra aloud | Full prompts, verbose guidance |
| Experienced (1) | Mantra silent | Reduced prompts, more Socratic questions |
| Owned (2) | Mantra internalized | Minimal prompts — "Breathe. Listen. Play." |
| Mastered (3) | Mantra autonomous | Silent — the student is the Troubadour now |

This is already designed in `IMPLEMENTATION_ROADMAP.md` Layer 3 (scaffolding → identity prompts). The AI tier system adds another dimension: not just *how much* the Troubadour speaks, but *how deeply* it understands.
...
|----------|---------|----------------|--------|
| **P0** | AI three-layer architecture (Souffle→Voix→Chant) | Workbook IS the guru | Medium (see `07_MINIMUM_AI_MODE.md`) |
...
| **P1** | wllama + LFM2.5-1.2B in-browser inference | Mantra silent (internalized guide) | Medium |
