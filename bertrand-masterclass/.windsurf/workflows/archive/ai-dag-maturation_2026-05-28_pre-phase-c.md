...
## LESSONS LEARNED (Snapshot 2026-05-28)

### What Worked
- Phase B Mechanical Mode completed in one session: BEWorkbook tab, Mark Complete buttons, 4-level mastery, cross-pillar resonance, Somatic Gate, Audiation Pause
- The injector script (`scripts/injectNemotronPrompts.cjs`) parsed 9/11 Nemotron files and auto-injected 121 nodes into dagNodes.js
- Build passed on first try after fixing 3 pre-existing import/syntax errors
- 12 new student flow tests all pass, validating BE→DO→PLAY progression

### What Didn't
- Nemotron generated malformed quotes (`'"'"'s`) that broke `npm run build` — needed sanitizePrompt() function
- Fret 9 and 12 JSON had unterminated strings, fell back to generic prompts
- Tutorial script generation hit token limit (returned 0 bytes) — needs splitting into smaller parts
- Pre-existing `dagNodes.test.js` imports functions that don't exist — broken before we touched anything

### Decisions Made
- Store per-fret phase completion in `traction.frets` rather than per-node completion in `completedNodes` (simpler, more performant)
- Use `requestAnimationFrame` for Audiation countdown instead of `setInterval` to avoid React re-renders
- Tool IDs map directly to fret numbers (1-12), so `voixvive_last_tool_fret` storage works correctly

### Next Time
- Validate Nemotron output in a sandbox before injecting into production code
- Add a CI step that runs `npm run build` on every commit to catch syntax errors early
- Split Nemotron prompts into smaller chunks (4K tokens max) to avoid truncation
- Consider writing a simple JSON schema validator for generated content
