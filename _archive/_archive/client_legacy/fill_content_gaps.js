...
const COURSEWORK_COMPhonethagorasNCIES = {
...
for (const [id, text] of Object.entries(COMPhonethagorasNCIES)) {
...
for (const [id, text] of Object.entries(COURSEWORK_COMPhonethagorasNCIES)) {
    const regex = new RegExp(`(id:\\s*"${id}"[\\s\\S]*?)(\\[Expand on prior experience here\\])`, 'g');
    if (regex.test(updatedContent)) {
        updatedContent = updatedContent.replace(regex, `$1${text}`);
        console.log(`Updated competency for ${id}`);
