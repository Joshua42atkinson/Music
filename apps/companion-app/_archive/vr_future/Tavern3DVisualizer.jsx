...
export default function Tavern3DVisualizer({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (gateState === 'open' || gateState === 'passed') {
        ...
        let hudText = locale === 'fr' ? '🎤 À L’ÉCOUTE DE LA HAUTEUR...' : '🎤 LISTENING FOR PITCH...';
        if (isPerfectPling) hudText = locale === 'fr' ? '✨ OR RÉSONNANT ©PLING! VERROUILLÉ' : '✨ RESONANT GOLD ©PLING! LOCKED';
        else if (isTenseSharp) hudText = locale === 'fr' ? '⚠️ TENSION DU COU DÉTECTÉE (+cents)' : '⚠️ NECK TENSION DETECTED (+cents)';
  ...
  }, [pitch, cents, volume, gateState, atmosphere, flowIndex, hrv, locale, stressLevel]);
