...
| Configure `voix-vive.com` domain in Vercel | Joshua | P1 | ✅ Done (May 25) |
...
| **AI Developer Integration (LM Studio + MCP)** | AI | P0 | ✅ Done (May 25) |
| - MCP Server with file edit tools | AI | P0 | ✅ Complete |
| - AIDeveloperChat component | AI | P0 | ✅ Complete |
| - useLMStudio hook for Qwen Coder | AI | P0 | ✅ Complete |
| - Documentation (AI_DEVELOPER_GUIDE) | AI | P1 | ✅ Complete |
...
| DNS: voix-vive.com → Vercel | Joshua | **P0** | ✅ Done (May 25) |
...
- [x] Add domain in Vercel project settings
- [x] Update DNS at Squarespace registrar
...
### Key Decisions — 2026-05-25 (Guitar Game Pedagogy Sprint)

| Date | Decision | Rationale |
|------|----------|-----------|
| 2026-05-25 | **No time-gates except the reveal flash** | The TAP countdown bar and HOLD progress ring create stress and rushing. Bertrand's pedagogy is goal-focused, not friction-focused. Only the REVEAL window stays timed (that IS the drill — pattern appears, disappears). TAP and HOLD become student-paced: submit when ready. |
| 2026-05-25 | **BE_STEP added before Phase 1** | Bertrand's "BE > DO > HAVE" sequence requires an intention-setting breath moment before any drill. A simple 20–30 second breath screen (no scoring, no mic required) precedes Phase 1. Student taps "I'm ready" to proceed. |
| 2026-05-25 | **SING NOW pulse added to Phase 2 gate-open** | At 50% orb descent (gate open), current text says "hear it in mind." Bertrand's PLING! protocol says sing FIRST. Explicit large-text pulse: "Sing it." replaces the passive prompt. |
| 2026-05-25 | **Somatic hit celebration on Phase 2 orb hit** | Correct orb taps currently disappear silently. Bertrand's 3rd Ear protocol ends with "celebrate." On correct hit: flash note name + brief cue ("That's A. Feel where it lives.") |
| 2026-05-25 | **GuitarHub.jsx created as /guitar landing** | Instead of dumping directly into VertiscaleEngine, /guitar now routes to a named game mode hub. 5 practices listed by Bertrand's protocol names. VertiscaleEngine moves to /guitar/vertiscale. |
| 2026-05-25 | **Four portals clarified as distinct instruments** | /song = Workbook (read, annotate theory). /guitar = Dojo (drills, muscle memory). /player = Body Lab (somatic prep, recording). /playbook = Integration (journal fed by game sessions). FHEAL journal will eventually hand off to /playbook instead of living inside VertiscaleEngine. |
| 2026-05-25 | **Difficulty names unified across all selectors** | OrbEngine already uses `awakening / practice / flow`. MenuScreen uses `beginner / standard / challenge`. Unify to awakening / practice / flow everywhere. |
| 2026-05-25 | **Bertrand audio plan identified** | Three short recordings needed from Bertrand: (1) 20-sec breath cue for BE_STEP, (2) 5 coaching sentences for FHEAL cues, (3) hit sound ("Oui." or "There it is.") for Phase 2 orb hits. Phone-quality acceptable. |

### Game Mechanics Audit — 2026-05-25

Full audit of existing game infrastructure documented. Key findings:

**What exists and works:**
- `GameFretboard.jsx` — 6-string interactive grid, 6 visual states (pattern/dark/tappable/hit/missed/phantom)
- `OrbEngine.jsx` — rAF-accurate descending orbs, audiation pause at 30%, mic gate at 50%
- `PitchGateUI.jsx` — real-time pitch needle + cents deviation
- `scoreCalculator.js` — 4 pure scoring functions: placement (35%), pitch (25%), breath (20%), consistency (20%)
- `sessionLogger.js` — Dexie + tractionStore dual persistence
- `BiometricSanctum` — headless HRV/flow/stress collection (runs hidden)
- Stage progression: 2 strings → 3 → 4 → 6 across 8 rounds (progressive overload)
- FHEAL Summary: coaching cues already written in Bertrand's voice, somatic progression SVG chart

**Gaps identified (priority order):**
1. TAP countdown bar creates stress — remove
2. No BE step before Phase 1
3. "Sing it" cue absent at Phase 2 gate open
4. No hit celebration in Phase 2
5. No dedicated CAGED Hunt game mode (data exists in vertiscalePatterns.js, no UI)
6. Theory Pillar has no game
7. FHEAL journal lives inside game (should eventually deposit to /playbook)
8. No orientation onboarding (Bertrand's Past/Future/Present playlists exercise)
