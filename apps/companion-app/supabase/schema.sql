...
CREATE POLICY "Authenticated users can view active drive config structure"
...
CREATE POLICY "Users can book available slots"
  ...
  USING (is_booked = FALSE OR auth.uid() = booked_by)
  WITH CHECK (
    booked_by = auth.uid()
    AND is_booked = TRUE
  );
...
CREATE POLICY "Authenticated users can write own audio sessions"
  ON public.troubadour_audio_sessions FOR INSERT
  WITH CHECK (auth.uid() = user_id); -- Use service_role key from middleware for cross-user writes
...
CREATE POLICY "Authenticated users can write paralinguistic events"
  ...
  ); -- Use service_role key from middleware for cross-user writes
