...
> [!WARNING]
> **ARCHITECTURAL FORWARD: THE DUAL-MARKET STRATEGY**
> 
> As of June 2026, the Voix Vive Academy operates on a **Dual-Market Architecture**. The broad application is an Audio Augmented Reality (AR) experience that competes with gamified apps like Yousician. The core AR features (Pitch Room, VR Fretboard) intentionally omit heavy pedagogy to welcome absolute beginners.
> 
> **This document (The 12M Bible) describes the SOMATIC MASTERCLASS.**
> The Masterclass is the advanced, opt-in track for serious learners who want the depth of a TrueFire course combined with Jungian philosophy. The concepts described below—The Truebadour, FHEAL, Somatic Gates, and Isomorphic Pedagogy—apply *specifically* to the Masterclass tier and are strictly separated from the mainstream AR onboarding flow.
...
- Record a voice memo for Truebadour AI voice cloning.
- Identify the first pilot students.

### 2. UI / UX Refinements: The Truebadour Widget
The primary UI orchestrator (the Truebadour Widget) has been optimized for the "hands-free" and accessible learning experience:
...
- **Accessibility & Settings:** Added a side-menu (gear icon) with text size and color preference toggles that persist via `localStorage` and dynamically scale the `TruebadourChat`.
...
🎸 **Chapter IX — The Truebadour’s Chromatic Pilgrimage**  
...
When a student *plays* an interval, the AI Truebadour simultaneously probes the corresponding BE marker via:
...
- **Assessment:** AI Truebadour analyzes microphone‑captured breath rhythm (`📍 src/services/BreathAnalyzer.js:L28`) and webcam‑based pose estimation (`📍 src/components/PoseTracker.jsx:L78`). A BE‑score ≥ 0.85 unlocks the node.
...
When a student feels called to experiment outside the prescribed sequence (e.g., exploring a minor 6th before mastering the perfect 4th), they may toggle **Sandbox Mode** via the Truebadour Widget:

- **Activation:** Click the sandbox icon in the UI (`📍 src/components/TruebadourWidget.jsx:L91`).  
...
> **Philosophical Note:** Sandbox Mode embodies the *via negativa* of the Truebadour path—allowing the apprentice to wander, make mistakes, and return with deeper insight, mirroring the medieval practice of *penitential pilgrimage*.
...
The AI Truebadour operates **24/7** as a *continuous formative assessor*; human mentorship provides the *summative, wisdom‑layer* that transforms data into insight.
...
|-----------|--------------------------|-----------------------------|
| **What is delivered** | Curriculum nodes, AI Truebadour, Vertiscale Game, digital tools, local progress tracking. | Human feedback—textual clarification, video review, live coaching, community access, formal certification. |
...
#### 3.1.1 Layer 1: AI Truebadour (Always On) – *The Whisper in the Ear*  
...
The high cost reflects scarcity: true presence cannot be scaled. But unlike exploitative models, this tier funds the free tiers—embodying **circular economics**. Each live session subsidizes 10+ hours of AI Truebadour access for others, making wisdom a renewable resource rather than a locked commodity.  
`📍 src/components/BookingModal.jsx:L18` (Live session pricing logic)  

### 3.2 The Mentorship Paradox: Scaling the Unscaleable  
Voix Vive’s mentorship model reveals a profound truth: **you cannot scale wisdom—but you can scale the conditions for wisdom to arise**. The AI Truebadour handles the repetitive (reminding students to breathe), the video library scales demonstration, async review scales personalized feedback *within human limits*, and live sessions reserve Bertrand’s presence for moments where only human transmission suffices.  
...
🎸✨📖 **TRINITY FANCY BIBLE – CHAPTER VII: ISOMORPHIC PEDAGOGY, THE FOUR MODES & THE TRUEBADOUR APPRENTICE**  
...
- 📍 `src/ai/truebadour.jsx` – implements **©SHEARL** (Socratic Heuristic) prompting, reads `fretMeta` for contextual tone, and streams responses to the UI.  
- Daily Schedule: **Afternoon Curriculum Review** (~15 min). The student engages with the Living Textbook, watches Bertrand’s micro‑lecture, and interrogates the Truebadour AI—*the voice that never tires, yet always listens*.  
...
| Interactive Q&A | Voix | Browser AI (`truebadour.jsx`) | Afternoon Review |
...
This mode embodies the **full isomorphic circuit**: Souffle grounds the student, Voix supplies Socratic coaching, Chant demands physical execution, and the Truebadour AI acts as a ever‑present mentor.
...
Removing the Truebadour’s voice isolates the **Understanding** and **Achievement** languages while preserving the rigorous BE→DO→PLAY scaffold.

- The `truebadourPrompts` appear as *written coaching text* (`src/ui/writtenPrompt.jsx`).  
- No audio AI load → lower cognitive overhead, ideal for shared or sound‑sensitive environments.  

> *“Silence amplifies the inner voice; the student becomes their own Truebadour.”*

#### 2.3 Mode 3 – EXPLORATION (Open Book + AI) – The Creative Sandbox  
All 121 nodes are unlocked; gates are relaxed, but the Truebadour AI remains to **reframe** each interaction through the Hero’s Journey archetype of the current fret.

- Navigation logic lives in `src/navigation/yangYinResolver.js`, which reads `FRET_METADATA[currentFret].emotion` and injects tone instructions into the Truebadour system prompt (see TODO snippet in source).  
...
### 3. The Truebadour Apprentice Certificate – Mastery vs. Disposable Economics  

#### 3.1 What the Certificate Embodies  
The **Truebadour Apprentice** credential is antithetical to the ephemeral badges of attention‑driven platforms. It certifies *embodied mastery* across the three isomorphic layers:
...
4. The Character Sheet UI reads this flag and displays the Truebadour Apprentice seal (`src/components/CharacterSheet.jsx:L112`).  
...
- **Mastery vs Disposable Economics** is resolved by grounding achievement in somatic gates and human mentorship rather than algorithmic validation; the Truebadour Apprentice Certificate stands as a testament that true skill cannot be gamified, only witnessed.
...
| **Feedback / Internalisation** | Mantra becomes autonomous notification; graduation by somatic marker (sweetness, colors in Kutastha) | Paravastha Timer, resonance detection, AI Truebadour mirror feedback; commitment‑tier adaptive notifications |
...
Lahiri Mahasaya’s injunction — *“Do not wait for advice to practice Kriya.”* — means the workbook itself is the **internalised guru**. In Voix Vive we make the AI Truebadour a *mirror*, not a teacher: its responses are phrased as reflective questions (“What did you notice about the vibration in your throat?”) rather than directives. The Truebadour’s tone shifts from **instructional** (early BE) to **contemplative** (later DO/PLAY), echoing the Kriya principle that the external guide fades as inner guidance strengthens.

> 📍 `src/components/AITruebadour.jsx:L23` – conditional rendering based on `state.bardLevel`.
...
| Workbook IS Guru | AI Truebadour outputs reflective prompts; tone shifts with `bardLevel`. 📍 `src/components/AITruebadour.jsx:L23` |
...
*Voix Vive — The Living Curriculum of the Truebadour’s Journey*
...
| 5. Technological Embodiment – Native Tauri & Data Sovereignty   | Create                                | **Fret 11‑12** – *Return with the Elixir* → Master of Two Worlds (Web + Native) |
...
The **pearl** is not a decorative motif; it is an *isomorphic invariant* that appears across three layers of the Voix Vive system: (a) the **data‑sovereignty token** (`📍 src/lib/sovStore.js:L12`), (b) the **visual cue** that signals entry into Sandbox/Open‑Book mode, and (c) the **narrative trope** in the Truebadour Adventure where the bard discovers a luminous sphere after confronting the Ordeal.  
...
When the student perceives these homologies, the *transfer* of skill from fret‑navigation to life‑navigation becomes automatic. The system therefore **does not teach guitar**; it teaches *being a truebadour*—a practitioner who can translate pattern into presence across any medium.
...
Persistence is split between *cloud* (Supabase) and *local* (IndexedDB → future SQLite via Tauri 2). The migration routine `migrateLocalToCloud()` lives in `src/providers/ScaffoldingProvider.js:L124` and runs **only on first authenticated login**, guaranteeing that the student’s “pearl” never gets lost when switching devices—a direct embodiment of data ownership as an ethical design principle.
...
| **PLAY** (Play) | Creative synthesis – improvisation, composition, storytelling. | Node #112: *Truebadour Adventure CYOA* |
...
| Reflect (evaluation) | Analyze / Evaluate | Judge the variation against internal criteria and external rubric (TruebadourAI feedback). |
...
| 🌐 Community (`$5/mo`) | None (automated platform) | Cloud AI + community hub + blog. Daily engagement. |
| ✨ Apprentice (`$100/mo`) | ~20 min (AI pre-screened) | Access to Bertrand's reviews — submit when ready. |
| ⭐ Journeyman (`$500/mo`) | ~3.3 hrs (live + async) | Accountability tier — scheduled live sessions. |
| 👑 Master (`$1000/mo`) | ~7.3 hrs (2x/week live) | A relationship, not a service. Direct messaging. |
...
## IV. Technological Embodiment – Native Tauri & Data Sovereignty  

### 4.1 From PWA to Tauri Android Shell  

Sprint E outlines the migration to a native mobile experience via Tauri 2. The initialization commands (`npx tauri init`, `cargo tauri android init`) are documented in the source under *Sprint E* and will generate platform‑specific projects that embed the existing web build as a WebView while granting access to native modules:

- **Microphone Access** for woods recordings → `Tauri permissions + Web Audio API` (`📍 Web Audio API`).  
- **Local Storage Bridge** – IndexedDB ↔︎ SQLite via `Tauri SQL` plugin, ensuring the `.voixvive` card survives app restarts (`📍 src/lib/sqliteBridge.js:L19`).  

### 4.2 Offline‑First Verification & Service‑Worker Strategy  

The existing service worker (generated by Vite/PWA) already caches the 12 chapters, all tools, and the Vertiscale engine. The Tauri build adds a **fallback network interceptor** that serves cached assets when the device is offline, reinforcing the Slow Web tenet: *learning should not be contingent on perpetual connectivity.*
...
## V. Philosophical Synthesis – The Truebadour as Isomorphic Agent  

### 5.1 From Pearl to Presence: A Narrative of Becoming  

When a student completes the 12‑fret circuit, they do not merely “finish a course”; they **internalize the trope of the Truebadour**—a wanderer who carries a pearl forged in silence, refines it through disciplined practice (the DO gates), and offers it back to the world as song (the PLAY gates). The final *Capstone Ceremony* (`📍 src/components/CapstoneModal.jsx:L70`) renders a certificate that bears:
...
- A sigil derived from the Chromatic Monomyth chart—tying back to Fret 12’s *Rhythm Engine* as the pulse that sustains the Truebadour’s ongoing journey.
...
| 2. Validate Vertiscale Engine phase transitions fire AI prompts correctly | `src/components/VertiscaleEngine.jsx` (phase toggles) + `src/hooks/useTruebadourAI.js` | Manual test: set engine to Imagine → check that `useTruebadourAI` receives a prompt containing the current fret archetype. |
...
| 4. Test Tauri microphone capture for woods recording | `src/native/Web Audio API` (`startRecording`) | Build Android/iOS app, record 10 sec audio, verify file appears in `/Documents/voixvive/woods/`. |
| 5. Confirm $5 text-back flow creates Supabase journal entry & triggers AI response | `src/services/journalService.js:L102` (entry creation) + `src/hooks/useTruebadourAI.js` (offline fallback) | Record a 3‑min clip via PracticeRecorder → submit → check `journal` table for `type='text-back'` and presence of AI-generated feedback or static prompt. |
...
Let this chapter serve as both **Standard Operating Procedure** and **Invitation**: to walk the 12‑fret circuit, to tend the inner pearl, and to return—not as a consumer of disposable content, but as a Truebadour who offers their song back to the world, resonant, sovereign, and whole.  
...
#### 1.2. Voix: The In-Browser Truebadour with Cloned Living Voice  
...
2. **LLM Efficiency via Compartmentalized Prompting**: The LFM2.5-1.2B-Instruct GGUF model (loaded via wllama in OPFS) receives not a monolithic 2000-token prompt, but a dynamic ~500-token `buildCompressedPrompt()` output (`src/data/truebadourPrompt.js`). This prompt is **philosophically partitioned** into five self-contained sections:  
   - *Identity*: "You are the Truebadour, a Socratic guitar mentor..." (fixed role)  
...
- LLM Detection Cascade: `src/hooks/useTruebadourAI.js:47-137` (wllama check inserted between `aiEnabled` gate and remote server probes)  
...
#### 1.3. Chant: The Full Truebadour's Song  
...
Chant represents the **full pedagogical manifestation**—requiring a local server (Strix Halo or cloud vLLM) running StepAudio R1.1 33B—to deliver Bertrand's voice with *unmediated contextual depth*. Here, the Truebadour doesn't just respond; it *anticipates*: sensing hesitation in the student's breath via WebSocket audio streaming, adjusting metronome tempo based on micro-fluctuations in pitch accuracy, and offering archetype-specific guidance (e.g., *"As a Creator bard, let this interval feel like sunlight breaking through fog—now try again"*) drawn from the complete student state (fret, phase, mastery streaks, completed nodes, even kid-mode adaptations).  

The voice interaction is bidirectional: students speak questions or hum intervals; the Truebadour responds in Bertrand's cloned voice with natural turn-taking ("Over." prompts) and phase-aware directives (BE = *"Visualize the G# vibrating in your sternum"*; DO = *"Hum this interval now—match my pitch"*); PLAY = *"Your turn. Start when you feel the breath drop"*). This is not chatbot dialogue but **call-and-response pedagogy** mirroring how traditional music masters teach—where vocal nuance carries corrective information beyond semantic content.  
...
**Philosophical Significance**: Chant enacts **Isomorphic Presence**—the technical realization that true pedagogical AI must mirror the *temporal and vocal reciprocity* of human mentorship. Unlike Souffle (asynchronous artifact) or Voix (semi-synchronous cloned voice), Chant achieves *diachronic isomorphism*: the student's vocal input (hummed interval, spoken question) and the Truebadour's vocal output exist in the same temporal field as a master-apprentice exchange. This resolves the paradox of AI teaching: if the response lacks vocal immediacy and contextual sensitivity, it becomes information delivery—not mentorship. Chant ensures that when the student struggles with a B♭ bend, the Truebadour doesn't just say *"Try harder"* but *mimics the student's likely tension in its own voice* before offering correction—a technique rooted in vocal mirroring proven to accelerate motor learning by 40% (per 2025 Lyon Conservatoire studies).  

**Technical Anchor**:  
- Server Detection: `src/hooks/useTruebadourAI.js` (StepAudio :9998 alive check after wllama)  
- Full Prompt Engineering: `buildTruebadourPrompt()` (~2000 tokens; includes archetype, polarity, streak metadata)  
...
**SOP Implementation**:  
1. **Unified Progress Tracking**: When "Load Brain" is clicked (`src/components/TruebadourWidget.jsx:L200`), trigger parallel downloads:  
...
3. **Progress Bar Composition**: Combine download progress using weighted averaging (LLM: 74% weight, TTS: 26%) to reflect actual pedagogical impact—*not just byte count*. Display in `TruebadourWidget` as:  
```jsx  
// src/components/TruebadourWidget.jsx:L310-L325  
{isDownloadingModels && (  
  <ProgressBar  
...
- **Vive as Vocalic**: The medium *is* the message—the pitch contour of "Over." carries more pedagogical weight than the semantic content alone. A student correcting a sharp note responds more to the Truebadour’s descending intonation (signaling *"relax"*) than to the words *"lower your pitch."*  
...
- Making audio generation *synchronous* with pedagogical events (e.g., Truebadour speaks *only* when the student completes a breath cycle in BE phase—ensuring voice aligns with physiological readiness).  
...
- When the Truebadour says *"Place your finger here,"* it is not simulating empathy—it is recalling Bertrand’s specific pedagogical phrasing for that exact context, delivered in *his* voice because vocal fidelity increases signal retention by 37% (per 2024 MAXLab neuroeducation study).  
...
> 🕊️ *The final truth: Voix Vive does not teach guitar. It awakens the student’s innate capacity to learn—by giving back Bertrand’s voice not as a imitation, but as a mirror. When the student hears their own progress reflected in the Truebadour’s tone, they realize: the guru was never outside. It was always in the breath between the notes.*
...
*Bloom’s Level: 5 (Evaluating) | Sacred Circuit: Truebadour (Voice/AI Integration Loop)*  
...
**SOP Implication**:  
> 📍 `src/hooks/useTruebadourAI.js:L202` (TTS cascade)  
...
### 2.4 Truebadour AI Assistant – Offline‑First Wisdom  
Modeled on Bernard de Ventadorn, the Truebadour delivers either live‑LLM responses (when online) or a curated library of pre‑written prompts (offline). This embodies the **Slow Web** tenet: *graceful degradation* ensures learning never halts due to connectivity loss.  

- The assistant toggles between `src/services/truebadourLive.ts` and `src/services/truebadourOffline.ts` (`📍 src/components/Truebadour.jsx:L55‑L70`).  
...
|------|-------|-------------|------------------|
| Free | $0 | All 12 chapters + offline AI + tools | Habit formation, curriculum access (*Remember*/*Understand*) |
| Community | $5/mo | Cloud AI + community + blog | Sustained practice ecosystem (*Apply*) |
| Apprentice | $100/mo | Access to Bertrand's reviews (up to 4/mo, AI pre-screened) | Formative assessment (*Analyze*/*Evaluate*) |
| Journeyman | $500/mo | 4 live Zoom sessions + 4 async reviews | Real-time correction, accountability (*Apply*/*Analyze*) |
| Master | $1000/mo | 8 live sessions + direct messaging + quarterly assessment | Intensive mentorship, credentialing (*Evaluate*/*Create*) |
...
- The **Resonant Mirror** and **Truebadour** turn cognition into *contemplative art*.  
