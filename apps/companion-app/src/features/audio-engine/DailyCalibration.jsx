...
export default function DailyCalibration({ onClose }) {
  ...
  const { isListening: _isListening, pitch, volume, noteInfo, startListening, stopListening } = usePitchDetector();
  const { updateTraction } = useScaffolding();

  const _stabilityTimerRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (inTune) {
      ...
      if (consecutiveInTuneFrames.current >= 8) {
        ...
        if (navigator.vibrate) navigator.vibrate(50); // Subtle haptic pop
  ...
  const handleSuccess = useCallback(() => {
    ...
    playPling(440);
    if (navigator.vibrate) navigator.vibrate([30, 50, 100]); // Success pattern
  ...
  return (
    <div className="fixed inset-0 z-[2000] bg-cf-void/98 backdrop-blur-xl flex flex-col justify-between p-6 md:p-8 text-cf-ink font-body">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(var(--cf-gold-rgb),0.08)_0%,transparent_70%)] pointer-events-none" />
      ...
      <div className="relative flex-1 flex flex-col items-center justify-center max-w-xl w-full mx-auto py-8 z-10">
        <AnimatePresence mode="wait">
          ...
          {step === 'intro' && (
            ...
            >
              <div className="w-16 h-16 rounded-full bg-cf-gold/10 border border-cf-gold/30 flex items-center justify-center text-cf-gold shadow-[0_0_30px_rgba(var(--cf-gold-rgb),0.1)]">
                <Volume2 size={32} />
              </div>
              <div>
          ...
          {step === 'tuning' && (
            ...
            >
              ...
              {/* Cumulative Tuning Progress */}
              <div className="flex items-center justify-center gap-3 mb-1">
                <div className="relative w-12 h-12">
                  <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                    <circle cx="18" cy="18" r="15" fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="3" />
                    <circle cx="18" cy="18" r="15" fill="none" stroke="var(--cf-gold)" strokeWidth="3"
                      strokeDasharray={`${tunedStrings.filter(Boolean).length / 6 * 94} 94`}
                      strokeLinecap="round"
                      className="transition-all duration-500 ease-out"
                      style={{ filter: 'drop-shadow(0 0 4px rgba(var(--cf-gold-rgb),0.3))' }}
                    />
                  </svg>
                  <span className="absolute inset-0 flex items-center justify-center font-mono text-[10px] text-cf-gold font-bold">
                    {tunedStrings.filter(Boolean).length}/6
                  </span>
                </div>
                <span className="font-mono text-[10px] text-cf-whisper uppercase tracking-widest">
                  {tunedStrings.filter(Boolean).length === 6 ? 'All strings in tune ✓' : 'Tuning in progress'}
                </span>
              </div>
                  ...
                  {STRINGS.map((str, idx) => {
                    ...
                    return (
                      <motion.button
                        ...
                        animate={isTuned ? { scale: [1, 1.15, 1] } : {}}
                        transition={{ duration: 0.4 }}
                        className={`py-3 rounded-lg font-mono text-sm font-bold border transition-all ${
                          isTuned 
                            ? 'bg-cf-sage/20 border-cf-sage text-cf-sage shadow-[0_0_16px_rgba(52,211,153,0.2)]'
                            : isCurrent
                              ? 'bg-cf-gold/10 border-cf-gold text-cf-gold shadow-[0_0_12px_rgba(var(--cf-gold-rgb),0.15)]'
                      ...
                      >
                        {isTuned ? (
                          <span className="flex flex-col items-center">
                            <span>✓</span>
                            <span className="text-[9px] font-normal mt-0.5">{str.note}</span>
                          </span>
                        ) : (
                          <>
                            {str.note}
                ...
                <div className="bg-cf-void border border-cf-border/30 rounded-xl p-4 flex flex-col gap-4 mt-2">
                  ...
                  <div className="relative py-4">
                    ...
                    <div className="relative h-2 bg-cf-surface rounded-full overflow-visible">
                      ...
                      {pitch && (
                        <motion.div
                          className="absolute top-[-6px] bottom-[-6px] w-1 bg-cf-gold shadow-[0_0_10px_var(--cf-gold)] rounded"
