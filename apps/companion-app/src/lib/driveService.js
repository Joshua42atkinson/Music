...
if (!MENTOR_EMAIL) {
  devWarn('[DriveService] VITE_MENTOR_EMAIL not set — mentor sharing disabled');
}
...
export async function shareWithMentor(fileId, role = 'writer') {
  if (!MENTOR_EMAIL) {
    devWarn('[DriveService] Cannot share — VITE_MENTOR_EMAIL not configured');
    return null;
  }
...
/**
 * Syncs a journal entry to the student's Google Drive archive.
 * Appends the entry to voix-vive-journal.json.
 */
export async function syncJournalToDrive(journalEntry) {
  const folderId = await getOrCreateSubmissionsFolder();
  const fileName = 'voix-vive-journal.json';
  const token = await getGoogleToken();
  if (!token) return null;

  // 1. Search for existing journal file
  const search = await driveFetch(
    `/files?q=${encodeURIComponent(
  ...
  // 2. Download existing content if present
  if (fileId) {
    try {
      ...
      devWarn('[DriveService] Could not read existing journal', e);
    }
  }
  ...
  const bodyContent = JSON.stringify(existingData, null, 2);
  ...
  if (fileId) {
    ...
    const boundary = '-------voix-vive-journal';
    const delimiter = `\r\n--${boundary}\r\n`;
    const closeDelimiter = `\r\n--${boundary}--`;
...
/**
 * Mark a submission as reviewed with mentor notes and an optional video response link.
 */
export async function markReviewed(submissionId, notes, mentorVideoLink = null) {
  if (!supabase) return null;
  const { data, error } = await supabase
    .from('video_submissions')
    .update({ 
      reviewed: true, 
      mentor_notes: notes, 
      mentor_video_link: mentorVideoLink,
