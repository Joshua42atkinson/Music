...
export async function saveR2SubmissionMetadata(userId, { exerciseName, videoUrl, duration }) {
  ...
  try {
    ...
    devWarn('[R2Service] Could not retrieve student profile ID:', err);
  }
...
export async function syncOutboxToR2(userId) {
  if (!supabase || !userId) {
    devWarn('[R2Service] Sync aborted: missing Supabase client or userId.');
  ...
  try {
    ...
    devLog(`[R2Service] Found ${queuedItems.length} queued items to sync to R2...`);
    let syncedCount = 0;
