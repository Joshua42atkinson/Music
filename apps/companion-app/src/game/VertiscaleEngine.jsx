...
function VertiscaleEngine({ onClose }) {
  ...
  const handleSessionComplete = useCallback(async (scores, logs) => {
    const _avgScore = scores.length > 0 ? scores.reduce((a, b) => a + b, 0) / scores.length : 0;

    const _avgHrv = biometricsHistory.length > 0
  ...
  const {
    flashState, tapProgressPct: _tapProgressPct, holdProgressPct,
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{
                ...
                textTransform: 'uppercase', color: 'var(--cf-gold)',
              }}>
                ⚡ VERTISCALE ENGINE
              </span>
              ...
              {activeBiometrics && (
                <div style={{
                  ...
                  <span style={{ color: 'var(--cf-gold)' }}>🫁 FLOW: {(activeBiometrics.flowIndex || 0).toFixed(2)}</span>
              ...
              <motion.div
                ...
                style={{
                  ...
                  background: avgScore >= 0.85
                    ...
                    : avgScore >= 0.5
                      ? 'linear-gradient(90deg, var(--cf-gold), #e8c84a)'
                      : 'linear-gradient(90deg, #cc5555, var(--cf-gold))',
                  boxShadow: `0 0 8px ${avgScore >= 0.85 ? 'rgba(46,213,115,0.4)' : 'rgba(var(--cf-gold-rgb),0.3)'}`,
            ...
            <motion.div key="phase1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <div style={{ textAlign: 'center', marginBottom: 16 }}>
                <p style={{
                  ...
                  letterSpacing: '0.15em', color: 'var(--cf-gold)', textTransform: 'uppercase',
              ...
              {round === 0 && flashState === FLASH_STATES.REVEAL && (
                <div style={{
                  background: 'rgba(var(--cf-gold-rgb),0.08)', border: '1px solid rgba(var(--cf-gold-rgb),0.2)',
                  borderRadius: 12, padding: '14px 18px', marginBottom: 14, lineHeight: 1.7,
                }}>
                  <p style={{ fontSize: '0.85rem', color: 'var(--cf-gold)', fontFamily: 'JetBrains Mono, monospace', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>
                    {t('ve_howToPlay')}
                  </p>
              ...
              {flashState === FLASH_STATES.HOLD && (
                <div style={{ height: 4, background: 'rgba(255,255,255,0.05)', borderRadius: 2, marginBottom: 12, overflow: 'hidden' }}>
                  <motion.div animate={{ width: `${holdProgressPct * 100}%` }} style={{ height: '100%', background: 'linear-gradient(90deg, #2ed573, var(--cf-gold))', borderRadius: 2, transition: 'width 100ms linear' }} />
                </div>
              )}
              ...
              {(flashState === FLASH_STATES.RESULT || flashState === FLASH_STATES.HOLD_RESULT) && roundScores.length > 0 && (
                <div style={{ textAlign: 'center', marginTop: 16 }}>
                  <p style={{ fontSize: '2.2rem', color: 'var(--cf-gold)', fontWeight: 300 }}>
...
function MenuScreen({ rootNote, setRootNote, phaseUnlock, onStart, micError, isListening, onStartMic, onClose, difficulty, setDifficulty, scaleType, setScaleType }) {
  ...
  const renderContent = () => (
    <div style={{ padding: '24px 16px', color: '#e8edf2' }}>
      ...
      <details style={{
        marginBottom: 20, background: 'rgba(var(--cf-gold-rgb),0.06)',
        border: '1px solid rgba(var(--cf-gold-rgb),0.15)', borderRadius: 12,
        overflow: 'hidden',
      }}>
        <summary style={{
          padding: '14px 18px', cursor: 'pointer', listStyle: 'none',
          fontFamily: 'JetBrains Mono, monospace', fontSize: '0.85rem',
          letterSpacing: '0.1em', color: 'var(--cf-gold)', textTransform: 'uppercase',
          display: 'flex', alignItems: 'center', gap: 8,
        }}>
      ...
      <div style={{
        ...
        {[
          ...
          <button key={d.key} onClick={() => setDifficulty(d.key)} title={d.desc} style={{
            ...
            fontFamily: 'JetBrains Mono, monospace', fontSize: '0.8rem',
            background: difficulty === d.key ? 'rgba(var(--cf-gold-rgb),0.15)' : 'rgba(255,255,255,0.03)',
            border: `1px solid ${difficulty === d.key ? 'rgba(var(--cf-gold-rgb),0.4)' : 'rgba(255,255,255,0.06)'}`,
            color: difficulty === d.key ? 'var(--cf-gold)' : '#5a6a80',
            transition: 'all 0.2s',
          }}>{d.label}</button>
        ))}
      </div>
      ...
      <div style={{
        ...
        {[
          ...
          <button key={s.key} onClick={() => setScaleType(s.key)} style={{
            ...
            fontFamily: 'JetBrains Mono, monospace', fontSize: '0.75rem',
            background: scaleType === s.key ? 'rgba(var(--cf-gold-rgb),0.15)' : 'rgba(255,255,255,0.03)',
            border: `1px solid ${scaleType === s.key ? 'rgba(var(--cf-gold-rgb),0.4)' : 'rgba(255,255,255,0.06)'}`,
            color: scaleType === s.key ? 'var(--cf-gold)' : '#5a6a80',
            transition: 'all 0.2s',
          }}>{s.label}</button>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: '100%', maxWidth: 400, margin: '0 auto' }}>
        <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.85rem', letterSpacing: '0.15em', color: 'var(--cf-gold)', textTransform: 'uppercase', margin: '0 0 2px' }}>
          {t('ve_innerFretboard')}
        </p>
        ...
        <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.85rem', letterSpacing: '0.15em', color: 'var(--cf-gold)', textTransform: 'uppercase', margin: '16px 0 2px' }}>
          {t('ve_innerEar')}
        </p>
        <p style={{ fontSize: '0.9rem', color: '#8090a8', margin: '0 0 8px', lineHeight: 1.6 }}>
          {t('ve_innerEarDesc')}
    ...
    >
      ...
      <motion.button
        ...
        style={{
          ...
          background: ready ? 'rgba(var(--cf-gold-rgb),0.12)' : 'transparent',
          border: `1px solid ${ready ? 'rgba(var(--cf-gold-rgb),0.4)' : 'transparent'}`,
          color: 'var(--cf-gold)', cursor: ready ? 'pointer' : 'default',
          fontFamily: 'JetBrains Mono, monospace', fontSize: '0.9rem',
  ...
  return (
    <motion.div key="phase2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <div style={{ textAlign: 'center', marginBottom: 8 }}>
        ...
        <p style={{ fontSize: '1rem', color: '#8090a8', marginTop: 8, lineHeight: 1.6 }}>
          {locale === 'fr' ? (
            <>Les notes descendent sur l'écran. Quand une note atteint la <span style={{ color: '#9b7acc' }}>ligne violette</span>, <strong>imaginez le son dans votre esprit</strong> — ne chantez pas encore. Quand elle traverse la <span style={{ color: 'var(--cf-gold)' }}>ligne dorée</span>, chantez dans le micro. Tapez sur l'orbe quand vous êtes prêt.</>
          ) : (
            <>Notes descend the screen. When a note hits the <span style={{ color: '#9b7acc' }}>purple line</span>, <strong>imagine the sound in your mind</strong> — don't sing yet. When it crosses the <span style={{ color: 'var(--cf-gold)' }}>gold line</span>, sing it into your mic. Tap the orb when you're ready.</>
          )}
        </p>
        ...
        {gateState === 'open' && !audiatePhase && (
          <motion.div
            ...
            style={{
              ...
              background: 'rgba(var(--cf-gold-rgb),0.1)', border: '1px solid rgba(var(--cf-gold-rgb),0.35)',
              marginBottom: 4,
            }}
          >
            <p style={{
              ...
              color: 'var(--cf-gold)', margin: 0, letterSpacing: '0.05em',
            }}>
      ...
      {audiatePhase && (
        ...
        >
          ...
          }}>{locale === 'fr' ? `🧠 ÉCOUTEZ D'ABORD — ${audiatePhase.note?.name}` : `🧠 HEAR IT FIRST — ${audiatePhase.note?.name}`}</p>
        ...
        <div style={{ display: 'flex', gap: 4, justifyContent: 'center', flexWrap: 'wrap' }}>
          {orbScores.map((s, i) => (
            <div key={i} style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              background: s.composite >= 0.7 ? 'rgba(46,213,115,0.2)' : s.composite > 0 ? 'rgba(var(--cf-gold-rgb),0.15)' : 'rgba(255,71,87,0.15)',
              border: `1px solid ${s.composite >= 0.7 ? 'rgba(46,213,115,0.4)' : s.composite > 0 ? 'rgba(var(--cf-gold-rgb),0.3)' : 'rgba(255,71,87,0.3)'}`,
      ...
      <button onClick={onBack} style={{
        ...
        color: 'var(--cf-gold)', cursor: 'pointer', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.9rem',
      }}>← Back to Menu</button>
    </motion.div>
  );
}
...
function PhaseButton({ label, desc, unlocked, progress, onClick }) {
  return (
    <button onClick={unlocked ? onClick : undefined} style={{
      ...
      background: unlocked ? 'rgba(var(--cf-gold-rgb),0.08)' : 'rgba(255,255,255,0.02)',
      border: `1px solid ${unlocked ? 'rgba(var(--cf-gold-rgb),0.25)' : 'rgba(255,255,255,0.05)'}`,
      ...
      <p style={{
        fontFamily: 'JetBrains Mono, monospace', fontSize: '0.85rem',
        letterSpacing: '0.1em', color: unlocked ? 'var(--cf-gold)' : '#5a6a80',
        margin: '0 0 6px',
      }}>{unlocked ? '✦' : '🔒'} {label}</p>
      ...
      {!unlocked && progress !== undefined && (
        <div style={{ marginTop: 8, height: 3, background: 'rgba(255,255,255,0.05)', borderRadius: 2 }}>
          <div style={{
            ...
            background: 'var(--cf-gold)', borderRadius: 2, transition: 'width 0.3s',
          }} />
        </div>
      )}
    </button>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.7rem', color: 'var(--cf-gold)', letterSpacing: '0.05em' }}>SOMATIC COHERENCE HISTORY</span>
        <span style={{ fontSize: '0.6rem', color: 'rgba(var(--cf-gold-rgb),0.5)', fontFamily: 'JetBrains Mono' }}>{sessions.length > 0 ? `${sessions.length} sessions` : 'simulation'}</span>
      </div>
      ...
      <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} style={{ overflow: 'visible' }}>
        ...
        <path d={getPathString(flowPoints)} fill="none" stroke="var(--cf-gold)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        {flowPoints.map((p, i) => (
          <circle key={`flow-${i}`} cx={p.x} cy={p.y} r="2.5" fill="var(--cf-gold)" stroke="#030306" strokeWidth="1" />
        ))}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.6rem', fontFamily: 'JetBrains Mono, monospace', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ width: 5, height: 5, borderRadius: '50%', background: 'var(--cf-gold)' }} />
          <span style={{ color: 'var(--cf-gold)' }}>Flow</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
...
function SummaryScreen({ roundScores, sessionLog, rootNote, avgScore, streakEligible, onClose, onRestart }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
      ...
      <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.85rem', letterSpacing: '0.15em', color: 'var(--cf-gold)', textTransform: 'uppercase' }}>
        {t('ve_innerVoice')}
      </p>
      ...
      <div style={{
        ...
        background: `conic-gradient(var(--cf-gold) ${avgScore * 360}deg, rgba(255,255,255,0.05) 0deg)`,
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}>
        <div style={{
          ...
          <span style={{ fontSize: '2rem', fontWeight: 300, color: 'var(--cf-gold)' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {roundScores.map((s, i) => (
              <div key={i} style={{ flex: 1, height: 48, background: 'rgba(255,255,255,0.03)', borderRadius: 4, position: 'relative', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', bottom: 0, width: '100%', height: `${s * 100}%`, background: s >= 0.85 ? 'rgba(46,213,115,0.4)' : 'rgba(var(--cf-gold-rgb),0.3)', borderRadius: '0 0 4px 4px' }} />
      ...
      <div style={{
        ...
        <p style={{ fontFamily: 'EB Garamond, serif', fontSize: '1rem', fontStyle: 'italic', color: 'var(--cf-gold)', lineHeight: 1.7 }}>
          "{coaching.text}"
        </p>
      ...
      <div style={{ width: '100%', maxWidth: 320 }}>
        <p style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.85rem', letterSpacing: '0.1em', color: 'var(--cf-gold)', marginBottom: 12, textAlign: 'center', textTransform: 'uppercase' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <style>{`
            ...
            .fheal-textarea:focus {
              ...
              border-color: rgba(var(--cf-gold-rgb),0.4) !important;
              box-shadow: inset 0 2px 10px rgba(0,0,0,0.2), 0 0 15px rgba(var(--cf-gold-rgb),0.15) !important;
            }
          `}</style>
        </div>
      </div>
