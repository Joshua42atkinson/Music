...
// ║ NEEDS   : usePitchDetector, truebadour.js, Web Speech API     ║
...
// Map high-fidelity TRUEBADOUR adventure to the linear Walking Mode scenes
const TRUEBADOUR_DATA = {
  scenes: Object.keys(TRUEBADOUR.scenes).reduce((acc, key) => {
    const s = TRUEBADOUR.scenes[key];
    acc[key] = {
      id: s.id,
...
export default function WalkingModeEngine() {
  ...
  const { isListening: _isListening, pitch, volume, startListening, stopListening } = usePitchDetector();
  ...
  const playScene = (targetSceneId) => {
    const scene = TRUEBADOUR_DATA.scenes[targetSceneId];
    if (!scene) {
  ...
  const advanceScene = () => {
    ...
    const scene = TRUEBADOUR_DATA.scenes[sceneId];
  ...
  }, [pitch, volume, sessionState, stopListening]);
