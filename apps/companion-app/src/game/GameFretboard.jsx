...
function GameFretboard({
  ...
  const cellStyle = () => {
    const base = {
      ...
      borderRight: '1.5px solid rgba(var(--cf-gold-rgb),0.12)',
  ...
  const dotStyle = (state, isRoot) => {
    const styles = {
      ...
      pattern:      { background: isRoot ? 'rgba(var(--cf-gold-rgb),0.7)' : 'rgba(var(--cf-gold-rgb),0.25)', border: `1.5px solid rgba(var(--cf-gold-rgb),${isRoot ? '1' : '0.6'})`, color: '#fff', boxShadow: `0 0 ${isRoot ? 18 : 10}px rgba(var(--cf-gold-rgb),${isRoot ? 0.7 : 0.4})` },
      ...
      'hold-pattern': { background: isRoot ? 'rgba(var(--cf-gold-rgb),0.5)' : 'rgba(var(--cf-gold-rgb),0.15)', border: `1.5px solid rgba(var(--cf-gold-rgb),${isRoot ? '0.8' : '0.4'})`, color: 'rgba(255,255,255,0.7)', boxShadow: `0 0 ${isRoot ? 14 : 8}px rgba(var(--cf-gold-rgb),${isRoot ? 0.5 : 0.25})` },
  ...
  return (
    <div className="rounded-xl py-3 border border-[rgba(74,51,36,0.6)] overflow-x-auto select-none"
      style={{ background: 'linear-gradient(180deg, #2a1a0e 0%, #1e1208 50%, #2a1a0e 100%)', boxShadow: 'inset 0 0 40px rgba(0,0,0,0.6)' }}
    >
      {STRING_TUNING.map((str, sIdx) => {
        const thickness = 0.8 + sIdx * 0.35;
        const brightness = Math.max(90, 180 - sIdx * 18);
        return (
          <div key={sIdx} className="flex items-center relative" style={{ height: CELL_H, borderBottom: '1px solid rgba(255,255,255,0.025)' }}>
            {/* String label */}
            <div className="text-center text-[0.85rem] font-bold text-[#8090a8] font-mono shrink-0" style={{ width: NUT_W, borderRight: '3px solid rgba(var(--cf-gold-rgb),0.5)' }}>
              {str.name}
            </div>

            {/* String line */}
            <div className="absolute top-1/2 right-0 z-0 pointer-events-none" style={{ left: NUT_W, borderBottom: `${thickness}px solid rgba(${brightness},${brightness-20},${brightness-40},0.45)` }} />
              ...
              if (isActivePitchCell && (state === 'pattern' || state === 'hold-pattern' || state === 'hit' || state === 'hold-hit' || state === 'tappable')) {
                if (Math.abs(cents) <= 15) {
                  ...
                  ds = {
                    ...
                    border: '2.5px solid var(--cf-gold)',
                    color: '#000',
                  ...
                  {ripple && ripple.sIdx === sIdx && ripple.fret === fret && (
                    <motion.div
                      ...
                      style={{
                        ...
                        border: '2px solid rgba(var(--cf-gold-rgb),0.5)', pointerEvents: 'none', zIndex: 3,
                      }}
                    />
                  )}
      ...
      <div className="flex mt-1" style={{ paddingLeft: NUT_W }}>
        {Array.from({ length: fretCount }, (_, f) => {
          const hasInlay = INLAY_FRETS.includes(f);
          const hasDouble = DOUBLE_INLAY.includes(f);
          return (
            <div key={f} className="shrink-0 text-center relative" style={{ width: CELL_W }}>
              <span className="text-[0.7rem] text-[#5a6a80] font-mono">
                {f === 0 ? 'Open' : f}
              </span>
              {hasInlay && (
                <div className="w-1.5 h-1.5 rounded-full mx-auto mt-[3px]"
                  style={{
                    background: 'radial-gradient(circle at 35% 35%, rgba(240,230,200,0.25), rgba(var(--cf-gold-rgb),0.1))',
                    boxShadow: '0 0 4px rgba(var(--cf-gold-rgb),0.15)',
                    border: '0.5px solid rgba(var(--cf-gold-rgb),0.1)',
                  }}
                />
              )}
              {hasDouble && (
                <div className="flex gap-1 justify-center mt-[3px]">
                  <div className="w-1.5 h-1.5 rounded-full" style={{ background: 'radial-gradient(circle at 35% 35%, rgba(240,230,200,0.25), rgba(var(--cf-gold-rgb),0.1))', border: '0.5px solid rgba(var(--cf-gold-rgb),0.1)' }} />
                  <div className="w-1.5 h-1.5 rounded-full" style={{ background: 'radial-gradient(circle at 35% 35%, rgba(240,230,200,0.25), rgba(var(--cf-gold-rgb),0.1))', border: '0.5px solid rgba(var(--cf-gold-rgb),0.1)' }} />
                </div>
              )}
            </div>
      ...
      {(flashState === FLASH_STATES.TAP || flashState === FLASH_STATES.RESULT || flashState === FLASH_STATES.HOLD_RESULT) && (
        <div className="flex gap-3 justify-center flex-wrap py-2 px-3 mt-1.5 rounded-md bg-white/[0.02] font-mono text-[0.7rem] text-[#5a6a80]">
          <span><span style={{ color: 'var(--cf-gold)' }}>●</span> Note</span>
