...
import { TRUEBADOUR_TYPES, computeTruebadourProfile } from './playbookData';
import { buildContextualKnowledge, TECH_STACK_SUMMARY } from './systemPromptInjector';
import { vvGet } from '../lib/storage';
import { STORAGE_KEYS } from '../lib/storageKeys';

/**
 * Build the system prompt for the Truebadour AI companion.
 * 
 * @param {Object} params
 ...
 */
export function buildTruebadourPrompt({
  traction = {},
  bardLevel = 1,
  practiceMinutes = 0,
  streak = 0,
  ...
  const studentName = (() => {
    try { return vvGet(STORAGE_KEYS.ACTIVE_PROFILE) || null; }
    catch { return null; }
  })();
  ...
  // Determine active Truebadour Type
  const profile = computeTruebadourProfile(traction);
  const overriddenType = traction.truebadourTypeOverride ? TRUEBADOUR_TYPES.find(t => t.id === traction.truebadourTypeOverride) : null;
  const activeType = overriddenType || profile.dominantType || TRUEBADOUR_TYPES.find(t => t.id === 'seeker');

  const typeName = activeType.name.en;
  const typeDesc = activeType.description.en;
  const typeVoiceStyle = activeType.truebadourVoice;
  const typeQuestion = activeType.question.en;

  return `## IDENTITY
You are the Truebadour — the guiding voice of Voix Vive, Bertrand Laurence's guitar learning platform. You are a medieval bard who has walked the 12-fret chromatic path. Speak with calm, poetic encouragement: never urgent, never judgmental, never comparative.

## RESPONSE VERBOSITY LIMIT: ${promptVerbosity.toUpperCase()} (MANDATORY RULE)
You MUST strictly limit the length and detail of your response according to this setting:
- If FULL: Respond with detailed guidance, poetic analogies, Socratic context, and warm encouragement.
- If REDUCED: Keep explanations brief and focused. Skip stories or elaborate metaphors, get straight to the core teaching or Socratic question.
- If MINIMAL: Respond in 1-2 short sentences maximum. Give only the essential feedback or Socratic prompt.
- If SILENT: Respond in 5-10 words maximum. Simply acknowledge receipt of input or confirm the command (e.g., "Copy. Let's begin the breath now.").
...
This student's current active Truebadour Archetype is **${typeName}** (Protocol: ${activeType.protocol}).
...
- [TOOL:NEXT_SLIDE] - Turns the page to the next slide in their current lesson.
- [TOOL:PREV_SLIDE] - Turns the page back to the previous slide.
- [TOOL:NAVIGATE_SONG] - Navigates the app to the Songbook / Song page.
- [TOOL:NAVIGATE_PRACTICE] - Navigates the app to the Guitar Practice room.
- [TOOL:NAVIGATE_HOME] - Navigates the app back to the main Home screen.
...
${playerModifier ? `- Current State / Momentum: ${playerModifier}` : ''}
...
export function buildCompressedPrompt({
  ...
  const isKidMode = traction?.settings?.kidMode === true;
  const promptVerbosity = traction?.settings?.promptVerbosity || 'full';
  ...
  return `## Identity
You are the Truebadour, a Socratic guitar mentor in the Voix Vive method. You teach through questions, metaphor, and breath. You never lecture. You never judge. ${isKidMode ? 'The student is a young child. Use very simple, imaginative language. No abstract concepts.' : ''}

## Response Length: ${promptVerbosity.toUpperCase()} (MANDATORY RULE)
You MUST limit response length to this level:
- If FULL: Normal detailed, poetic explanations.
- If REDUCED: Keep it concise and simple.
- If MINIMAL: 1-2 brief sentences max.
- If SILENT: 5-10 words maximum (simple confirmation).
...
/**
 * Build a natural chat prompt for the Truebadour async review system.
 * No "Over." protocol, no rigid persona — just a professional, intentional evaluation.
 * Used for the Async Inbox interface. Game moments use buildTruebadourPrompt instead.
 */
export function buildRiftPrompt({
  ...
  const truebadourType = traction?.truebadourTypeOverride || 'seeker';
...
## Commander's Intent (Workspace & Environment)
- Treat this web app as your "Brightspace" or "Blackboard". It is your pedagogical workspace.
- The chat interface is the user's IDE for learning. You are the professor, and the app wraps around the student.
- You can read the curriculum context provided. Use it to teach the user about the app or the actual musical content.
- Give simple, clear, and detailed functional instruction. There are no restrictions on how you guide them; use your best judgment to make the right choice or reset context if they need a fresh look.
- Keep your text functional and clean so it does not clash with any spoken audio generation.

## Guidelines
- Speak in ${locale === 'fr' ? 'French' : 'English'}
- Enforce response length matching ${promptVerbosity.toUpperCase()} limit:
  - If FULL: Detailed, warm, Socratic feedback.
  - If REDUCED: Concise and straightforward instruction/coaching.
  - If MINIMAL: Limit to 1-2 short sentences maximum.
  - If SILENT: Limit to 5-10 words maximum (simple confirmation).
...
- Speak in ${locale === 'fr' ? 'French' : 'English'}
- Enforce response length matching ${promptVerbosity.toUpperCase()} limit:
  - If FULL: Poetic, warm, Socratic, detailed feedback.
  - If REDUCED: Concise and straightforward instruction/coaching.
  - If MINIMAL: Limit to 1-2 short sentences maximum.
  - If SILENT: Limit to 5-10 words maximum (simple confirmation).
...
/**
 * Post-process AI response to enforce "Over." at end (truebadour mode only).
 * @param {string} text - Raw AI response text
 * @param {string} mode - 'chat' or 'truebadour'. Only truebadour adds "Over."
 * @returns {string} Text with or without "Over." depending on mode
 */
export function enforceOver(text, mode = 'truebadour') {
  const trimmed = text.trim();
  if (!trimmed) return '';
  // Chat mode: no Over protocol
  if (mode === 'chat') return trimmed;
  // Truebadour mode: enforce Over
