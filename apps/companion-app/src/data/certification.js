...
// The Truebadour's Trial: prove mastery through expression.
...
export const CERTIFICATION_TIERS = {
  apprentice: {
    ...
    requirement: 'Complete Chapter 1–4 (all phases, all pillars)',
  ...
  journeyman: {
    ...
    requirement: 'Complete Chapter 1–8, mastery ≥ 2 on all completed phases',
  ...
  master: {
    name: 'Bertrand Approved Truebadour',
    epithet: 'The Knowing Singer',
    requirement: 'Complete all 12 chapters, mastery ≥ 3 on at least 6 phases',
...
export function getNextCertificationGoal(traction) {
  ...
  if (!tier) {
    return {
      tier: 'apprentice',
      remaining: 'Complete Chapter 1 BE phase to begin',
    };
  }
  ...
  if (tier === 'apprentice') {
    return {
      tier: 'journeyman',
      remaining: `Complete Chapters 5–8 and raise all phase mastery to ≥ 2 (${status.progress.completedFrets}/8 chapters, ${status.progress.highMasteryCount}/${status.progress.totalPhases} high masteries)`,
    };
  }
  ...
  if (tier === 'journeyman') {
    return {
      tier: 'master',
      remaining: `Complete Chapters 9–12 and achieve mastery ≥ 3 on 6+ phases (${status.progress.completedFrets}/12 chapters, ${status.progress.highMasteryCount}/6 high masteries)`
    };
  }
