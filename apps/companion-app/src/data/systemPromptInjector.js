...
import { getFixFor, getKnownBugsFor } from './systemDebugging';
...
- AI Mentor (Truebadour): Knows student's archetype, adapts coaching tone to their somatic polarity
...
export function buildContextualKnowledge(studentState) {
  const { currentFret, currentPhase, truebadourType, backend } = studentState || {};
  const pe = PSYCHOLOGICAL_ENGINEERING;
  let parts = [];
...
Adjust instructions for ${currentPhase} phase.`);
  ...
  if (truebadourType) {
    const archetype = pe.archetypes[truebadourType];
    if (archetype) {
      parts.push(`STUDENT ARCHETYPE: ${truebadourType}
${getArchetypePrompt(truebadourType)}`);
    }
  }
