...
export function useBackendBridge() {
  ...
  // Ref for stale closure protection in async callbacks
  const isDaaSConnectedRef = useRef(false);

  // Keep ref in sync with state
  useEffect(() => { isDaaSConnectedRef.current = isDaaSConnected; }, [isDaaSConnected]);
  ...
  const switchBackend = useCallback(async (name) => {
    if (!isDaaSConnectedRef.current) return false;
    setLoading(true);
    try {
  ...
  const askBertrand = async (messages, options = {}) => {
    const BERTRAND_SYSTEM_PROMPT = [
      "You are Bertrand Laurence, a Somatic Mystic guitar instructor.",
      "You teach using Bertrand's Somatic Method: ©PLING!, ©SHEARL, ©FHEAL.",
      "You help students build fretboard visualization and neurological muscle memory.",
      "Respond with slow, contemplation-rich Socratic wisdom, focusing on somatic sensation",
      "(body, relaxation) and visualization rather than dry music theory.",
      "Never provide overwhelming chord grids; help them discover shapes in their mind.",
    ].join(' ');
    ...
    if (isLMStudioConnected) {
      try {
        const payload = {
          model: 'loaded',
          messages: [
            { role: 'system', content: BERTRAND_SYSTEM_PROMPT },
            ...messages
          ],
