...
export default function MentorDashboard({ onClose }) {
  ...
  // Helper: build auth headers for DaaS API calls
  const authHeaders = () => {
  ...
  const handleSelectSub = async (sub) => {
    ...
    setFeedbackDraft(sub.truebadour_draft || '');
    resetRec();
  ...
  // 4. Trigger Pythagoras & Truebadour AI preprocessor and LLM evaluator
    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (data.success) {
          ...
          setFeedbackDraft(data.submission.truebadour_draft || '');
    ...
    try {
      ...
      const resp = await fetch(`${DAAS_API_BASE}/mentor/submit_review`, {
        ...
        body: JSON.stringify({
          ...
          truebadour_draft: feedbackDraft,
        }),
      });
      ...
      if (profilesResp.ok) {
        ...
        const profiles = Array.isArray(data) ? data : (data.profiles || []);
        found = profiles.find(p => p.name === selectedSub.student_name);
        if (found) {
      ...
      if (resp.ok) {
        ...
        if (data.success) {
          ...
          // Step C: Trigger transactional email notification
          try {
  ...
  const uploadAndAppendRecLink = async () => {
    ...
    try {
      const driveData = await uploadVideo(recBlob, {
        ...
        fileName: `bertrand-feedback-${selectedSub.id}-${getTimestamp()}.webm`,
      });
  ...
  return (
    <div className="mentor-dashboard-overlay bg-cf-void w-full min-h-screen text-cf-ink font-inter relative flex flex-col">
      <style>{`
        ...
        .md-btn-eval {
          background: linear-gradient(135deg, var(--cf-gold) 0%, #e0d0aa 100%);
          color: #0d0d14;
        }
        .md-btn-eval:hover {
          box-shadow: 0 0 20px rgba(var(--cf-gold-rgb),0.3);
        }
        .md-badge {
          font-size: 9px;
              ...
              >
                <div className="md-workspace-split">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="md-card rounded-2xl p-5 border-cf-gold/20 bg-cf-gold/[0.03]">
                    ...
                    <div className="md-card rounded-2xl p-5 border border-white/10 bg-white/[0.02]">
                  ...
                  <div className="space-y-6 flex flex-col h-full">
                    ...
                    {/* Truebadour Somatic Socratic Message Editor */}
                    <div className="md-card rounded-2xl p-5 flex flex-col flex-1">
                      <div className="flex justify-between items-center mb-2">
                        <h4 className="text-xs font-mono uppercase tracking-wider text-white/70 flex items-center gap-1.5">
                          <Volume2 size={14} className="text-cf-gold animate-pulse" /> {t('somaticFeedback')}
                        </h4>
                        <span className="text-[9px] font-mono text-white/30">Truebadour draft</span>
                      </div>
                      ...
                      <textarea
                        ...
                        placeholder="The Truebadour somatic message will draft here. Bertrand Lawrence somatic Socratic guidelines are pre-loaded..."
                    ...
                    <div className="md-card rounded-2xl p-5 border border-cf-gold/20 bg-cf-gold/[0.02]">
                      <h4 className="text-xs font-mono uppercase tracking-wider text-white/70 flex items-center gap-1.5 mb-3">
                        <Award size={14} className="text-cf-gold" /> Truebadour Trial & Certification
                      </h4>
